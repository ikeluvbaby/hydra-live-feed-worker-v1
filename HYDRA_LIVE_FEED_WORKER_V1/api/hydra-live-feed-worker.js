...
async function postToHydra(payload) {
  ...
  const webhook = requireEnv("HYDRA_APPS_SCRIPT_WEBHOOK");
...
async function runLiveFeed() {
  ...
  if (!boolEnv("HYDRA_DRY_RUN", false)) requireEnv("HYDRA_APPS_SCRIPT_WEBHOOK");
...
export default async function handler(req, res) {
  try {
    const secret = req.query?.secret || req.headers["x-hydra-secret"];
    const expected = process.env.HYDRA_WORKER_SECRET;
    if (expected && secret !== expected) return res.status(401).json({ status: "ERROR", message: "Unauthorized." });
    if (req.query?.mode === "config" || req.query?.config === "1") return res.status(200).json(configCheck());
